...
const WeatherSettings = () => {
  ...
  useEffect(() => {
    const loadWeatherSettings = async () => {
      try {
        const response = await fetch("http://localhost:8001/settings/get?module=weather", {
          method: "GET",
        });
        if (!response.ok) {
  ...
  const handleSaveWeatherSettings = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost:8001/settings/save", {
        method: "POST",
  ...
  const testWeatherAPI = async () => {
    try {
      const response = await fetch("http://localhost:8001/weather/test", {
        method: "GET",
      });
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
